...
struct Ray{
    ...
    Ray(vec2 pos, vec2 dir) : x(pos.x), y(pos.y), r(sqrt(pos.x * pos.x + pos.y * pos.y)), phi(atan2(pos.y, pos.x)), dr(dir.x), dphi(dir.y) {
        ...
        double dt_d_lambda = sqrt( (dr*dr)/(f*f) + (r*r*dphi*dphi)/f );
        E = f * dt_d_lambda;
        // step 4) start trail :
        trail.push_back({x, y});
    }
...
void geodesicRHS(const Ray& ray, double rhs[4], double rs) {
    ...
    // d²r/d_lambda² from Schwarzschild null geodesic:
    double dt_d_lambda = E / f;
    rhs[2] = 
        - (rs/(2*r*r)) * f * (dt_d_lambda*dt_d_lambda)
        + (rs/(2*r*r*f)) * (dr*dr)
        + (r - rs) * (dphi*dphi);

    // d²φ/d_lambda² = -2*(dr * dphi) / r
    rhs[3] = -2.0 * dr * dphi / r;
}
